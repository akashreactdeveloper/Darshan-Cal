  ...
  try {
    ...
    if (section.sectionItems && section.sectionItems.length > 0) {
      return res.status(400).json({
        success: false,
        message:
          "Bulk upload already completed for this section. You can upload only once.",
      });
    }

    let createdItems = [];

    // Extract the first key from the data object (e.g., "0")
    const bulkData = data["0"];

    // Process Videos (Segments)
    if (bulkData && bulkData.segments) {
      const videoDocs = await Video.insertMany(
        bulkData.segments.map((segment) => ({
      ...
      const videoItems = videoDocs.map((video) => ({
        itemId: video._id,
        itemType: "Video",  // Correctly set the type to "Video"
      }));
    ...
    if (bulkData && bulkData.questions) {
    ...
    // Link Assessments to the Section
    const assessmentItems = createdItems.map((assessment) => ({
      itemId: assessment._id,
      itemType: "Assessment",  // Correctly set the type to "Assessment"
    }));

    section.sectionItems.push(...assessmentItems);

    // **Now, update the Section with new items (Only if Upload is Successful)**
    if (createdItems.length > 0) {
      await Section.findByIdAndUpdate(sectionId, {
        $push: {
          sectionItems: createdItems.map((item) => ({
            itemId: item._id,
            itemType: item instanceof Video ? "Video" : "Assessment",  // Dynamically set the itemType
          })),
        },
      });
    }
